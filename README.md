...
- **Обнаружение поворотов** - детектирование резких поворотов на 90° и плавных изгибов траектории
- **Мультирегиональное сканирование** - анализ линии в трех зонах для прогнозирования поворотов
...
Система использует продвинутый алгоритм с мультирегиональным сканированием:

### Базовое обнаружение

1. **Конвертация в 1-бит** - каждый пиксель становится либо 0 (черный), либо 255 (белый) на основе порога
2. **Мультирегиональное сканирование** - изображение делится на три горизонтальных региона:
   - **Верхний** (дальняя зона) - для раннего обнаружения поворотов
   - **Средний** - основная зона обнаружения
   - **Нижний** (ближняя зона) - наиболее надежная детекция
3. **Поиск от краев** - сканирование слева направо и справа налево для поиска линии в каждом регионе
4. **Вычисление центра** - центр линии = среднее между левым и правым краем

### Обнаружение поворотов

5. **Анализ смещения** - сравнение положения линии между регионами
...
Подробнее см. [CURVE_DETECTION.md](CURVE_DETECTION.md)
...
- **Время обработки кадра**: ~40-50ms (включая захват, бинаризацию, мультирегиональное детектирование)
- **Частота обновления**: ~8-10 FPS
