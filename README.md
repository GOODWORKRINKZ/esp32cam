...
- **🆕 1-битная монохромная обработка**: Ретро-стиль для максимальной скорости (в 2 раза быстрее, в 8 раз меньше памяти!)
- **🆕 Минимальное разрешение**: QQVGA (160×120) для сверхбыстрого детектирования
...
├── src/
│   ├── main.cpp                     # Основной код программы (веб-интерфейс)
│   ├── MonochromeLineDetection.h    # 1-битная монохромная библиотека
│   └── MonochromeLineDetection.cpp  # Реализация 1-битной обработки
├── examples/
│   ├── simple_line_detection.ino         # Простое обнаружение линии
│   ├── motor_control_integration.ino     # Интеграция с моторами
│   ├── monochrome_1bit_detection.ino     # 🆕 1-битное обнаружение
│   └── monochrome_1bit_robot.ino         # 🆕 1-битный робот
├── MONOCHROME_1BIT.md               # 🆕 Документация по 1-битной обработке
...
1. **🆕 Используйте 1-битную монохромную обработку** (см. [MONOCHROME_1BIT.md](MONOCHROME_1BIT.md))
2. **🆕 Минимальное разрешение QQVGA** (160×120) - в 2 раза быстрее!
3. Используйте QVGA (320x240) или CIF (400x296) для обычной обработки
4. Увеличьте Quality до 15-20
5. Отключите Denoise
6. Используйте Grayscale эффект
...
- [🆕 1-битная монохромная обработка](MONOCHROME_1BIT.md) - подробная документация

## 🆕 Новая возможность: 1-битная монохромная обработка

Теперь доступна **ультрабыстрая** 1-битная (бинарная) обработка изображений, вдохновленная ретро-камерами!

### Преимущества:
- ⚡ **В 2 раза быстрее** обычной обработки в градациях серого
- 💾 **В 8 раз меньше памяти** (2.4 КБ вместо 19.2 КБ для QQVGA)
- 🎯 **Идеально для контрастных линий** (черная линия на белом фоне)
- 🚀 **30+ FPS** на минимальном разрешении (160×120)
- 🤖 **Идеально для скоростных роботов**

### Примеры использования:
```cpp
#include "MonochromeLineDetection.h"

MonochromeLineDetection detector(128);  // Порог бинаризации
MonoLineResult result = detector.detectLine(fb);

if (result.detected) {
    Serial.printf("Линия на позиции %d%%\n", result.position);
}
```

### Быстрый старт:
1. См. пример: `examples/monochrome_1bit_detection.ino`
2. Для робота: `examples/monochrome_1bit_robot.ino`
3. Документация: [MONOCHROME_1BIT.md](MONOCHROME_1BIT.md)

### Когда использовать:
- ✅ Высококонтрастные сценарии (черное на белом)
- ✅ Скоростные роботы, требующие быстрой реакции
- ✅ Ограниченная память
- ✅ Простые линии на однородном фоне

Подробнее читайте в [документации по 1-битной обработке](MONOCHROME_1BIT.md)!
